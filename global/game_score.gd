...
func add_to_score_clear_rows(simultaneously_cleared_rows: int, modifiers: Array[Dictionary]) -> void:
	...
	for modifier in modifiers:
		...
		score_expression += modifier_operation + str(modifier_value)
	var base_score: float = score_expression.parse(score_expression).execute()
	
	var total_score = pow(base_score, 1.0 + (simultaneously_cleared_rows * EXPONENT_FACTOR))
